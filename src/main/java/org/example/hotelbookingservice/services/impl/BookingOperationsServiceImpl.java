...
import org.example.hotelbookingservice.dto.response.booking.operations.BookingItemResponse;
...
import org.example.hotelbookingservice.dto.response.booking.operations.CommissionSummary;
...
import org.example.hotelbookingservice.dto.response.booking.operations.PaymentSummary;
import org.example.hotelbookingservice.dto.response.booking.operations.PromotionSummary;
import org.example.hotelbookingservice.dto.response.booking.operations.RoomTypeSummary;
import org.example.hotelbookingservice.dto.response.booking.operations.UserSummary;
...
@Slf4j
public class BookingOperationsServiceImpl implements IBookingOperationsService {
    ...
    private final BookingOperationsRepository bookingOperationsRepository;
    ...
    public BookingResponse createBooking(UUID hotelId, BookingCreateRequest request, Authentication authentication) {
        ...
        requireAction(user.accountId(), "bookings.create", hotelId);
        ...
        bookingOperationsRepository.update("""
    ...
    public PaginatedResponse<BookingResponse> listMine(int limit, int offset, String status, Authentication authentication) {
        ...
        long total = bookingOperationsRepository.queryForObject("select count(*) " + where, params, Long.class);
        List<BookingResponse> rows = bookingOperationsRepository.query("""
                select b.*
                """ + where + """
    ...
    public PaginatedResponse<BookingResponse> listHotelBookings(UUID hotelId, int limit, int offset, String status, String q, Authentication authentication) {
        ...
        if (query != null) {
            where += "  and (lower(b.guest_name) like :q or lower(b.guest_email) like :q or lower(b.guest_phone) like :q)\n";
    ...
    public BookingResponse updateStatus(UUID hotelId, UUID bookingId, String requestedStatus, Authentication authentication) {
        ...
        switch (nextStatus) {
            case "CANCELLED" -> transitionToCancelled(booking, !hasSuccessfulPayment(bookingId));
              ...
              case "CONFIRMED" -> confirmPendingBooking(booking);
    ...
    public BookingResponse checkIn(UUID hotelId, UUID bookingId, CheckInRequest request, Authentication authentication) {
        ...
        UUID checkInId = bookingOperationsRepository.upsertCheckIn(bookingId, user.accountId(), trimToNull(request.note()));
    ...
    public PaymentStartResponse startPayment(UUID bookingId, PaymentStartRequest request, Authentication authentication) {
        if (!vnpayProperties.enabled()) {
            throw new ResponseStatusException(HttpStatus.SERVICE_UNAVAILABLE, "VNPAY payment is currently unavailable");
        }
        CurrentUser user = requireUser(authentication);
        BookingForUpdate booking = lockBooking(bookingId);
        if (!booking.accountId().equals(user.accountId())) {
    ...
    public String handleVnpayReturn(Map<String, String> params) {
        ...
        return UriComponentsBuilder.fromUriString(frontendProperties.paymentResultUrl())
    ...
    private String buildVnpayPaymentUrl(
        ...
        return VnpaySigner.buildPaymentUrl(vnpayProperties.payUrl(), params, vnpayProperties.hashSecret());
    }
    ...
    private String generateMerchantTxnRef(UUID bookingId) {
        ...
        while (true) {
            ...
            Boolean exists = bookingOperationsRepository.queryForObject("""
    ...
    private void markPaymentSucceeded(PaymentForCallback payment, Map<String, String> params) {
        int updated = bookingOperationsRepository.update("""
                update payments
    ...
    private void createLatePaymentReconciliationNotifications(PaymentForCallback payment) {
        List<UUID> adminIds = bookingOperationsRepository.query("""
    ...
    private void notifyHotelOperators(UUID hotelId, String type, String title, String body, String linkUrl) {
        List<UUID> recipients = bookingOperationsRepository.query("""
    ...
    public void expireDuePendingBookings() {
        List<UUID> ids = bookingOperationsRepository.query("""
                select id
                from bookings
    ...
    public void expireBookingIfDue(UUID bookingId) {
        List<BookingForUpdate> rows = bookingOperationsRepository.query("""
                select *
                from bookings
                where id = :bookingId
                  and status = 'PENDING'
      ...
      private void confirmPendingBooking(BookingForUpdate booking) {
          ...
          bookingOperationsRepository.update("""
                  update bookings
                  set status = 'CONFIRMED', pending_expires_at = null, updated_at = now()
                  where id = :bookingId
                    and status = 'PENDING'
                  """, new MapSqlParameterSource("bookingId", booking.id()));
      }
      ...
      private void completeCheckout(BookingForUpdate booking) {
        ...
        UUID checkInId = bookingOperationsRepository.existingCheckInId(booking.id());
        if (checkInId == null) {
            throw badRequest("Checkout requires an existing check-in");
        }
        bookingOperationsRepository.markCheckedOut(checkInId);
        bookingOperationsRepository.update("""
                update bookings
    ...
    private void reserveInventory(UUID hotelId, LocalDate checkIn, LocalDate checkOut, List<BookingItemRequest> items) {
        for (BookingItemRequest item : items) {
            ...
            while (date.isBefore(checkOut)) {
                int updated = bookingOperationsRepository.reserveInventory(hotelId, item.roomTypeId(), date, item.quantity());
                if (updated != 1) {
    ...
    private void releaseInventory(UUID bookingId, LocalDate checkIn, LocalDate checkOut) {
        List<BookingItemQuantity> items = bookingOperationsRepository.query("""
        ...
        for (BookingItemQuantity item : items) {
            LocalDate date = checkIn;
            while (date.isBefore(checkOut)) {
                bookingOperationsRepository.releaseInventory(item.roomTypeId(), date, item.quantity());
                date = date.plusDays(1);
            }
        }
    }
    ...
    private BookingResponse scopedHotelBookingDetail(UUID hotelId, UUID bookingId) {
        return bookingOperationsRepository.query("""
                select *
                from bookings
    ...
    private CheckInSummary loadCheckInSummary(UUID bookingId) {
        return bookingOperationsRepository.loadCheckInSummary(bookingId);
    }

    private List<BookingGuestResponse> loadGuests(UUID checkInId, UUID bookingId) {
        return bookingOperationsRepository.loadGuests(checkInId, bookingId);
    }
    ...
    private List<RoomTypeForBooking> loadRoomTypes(UUID hotelId, List<BookingItemRequest> items) {
        ...
        List<RoomTypeForBooking> rows = bookingOperationsRepository.query("""
    ...
    private PromotionForBooking resolvePromotion(UUID hotelId, UUID accountId, String rawCode, BigDecimal subtotal) {
        ...
        var repositoryPromotion = bookingOperationsRepository.findActivePromotionForBooking(hotelId, code, subtotal, now());
        if (repositoryPromotion.isPresent()) {
            PromotionForBooking promotion = mapPromotion(repositoryPromotion.get(), subtotal);
            if (promotion.perUserUsageLimit() != null) {
                long usedByAccount = bookingOperationsRepository.countPromotionUsesByAccount(accountId, promotion.id());
        ...
        List<PromotionForBooking> promotions = bookingOperationsRepository.query("""
                select *
                from promotions
        ...
        if (promotion.perUserUsageLimit() != null) {
            long usedByAccount = bookingOperationsRepository.queryForObject("""
                    select count(*)
                    from bookings
    ...
    private PromotionForBooking mapPromotion(Promotion promotion, BigDecimal subtotal) {
        BigDecimal discount = switch (promotion.getDiscountType()) {
            case "PERCENT" -> subtotal.multiply(promotion.getDiscountValue()).divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
        ...
        return new PromotionForBooking(promotion.getId(), discount.setScale(2, RoundingMode.HALF_UP), promotion.getPerUserUsageLimit());
    }

      private CommissionSnapshot resolveCommission(UUID hotelId, BigDecimal total) {
          var repositoryCommission = bookingOperationsRepository.findCommissionForHotel(hotelId);
          if (repositoryCommission.isPresent()) {
              var commission = repositoryCommission.get();
              BigDecimal rate = Boolean.TRUE.equals(commission.getActive()) ? commission.getCommissionRate() : BigDecimal.ZERO;
              BigDecimal amount = total.multiply(rate).setScale(2, RoundingMode.HALF_UP);
              return new CommissionSnapshot(commission.getCode(), rate, amount);
          }
          return bookingOperationsRepository.query("""
      ...
      private void consumePromotion(UUID promotionId) {
          bookingOperationsRepository.incrementPromotionUsedCount(promotionId, now());
      }

      private void restorePromotionUsage(UUID promotionId) {
          bookingOperationsRepository.decrementPromotionUsedCount(promotionId, now());
      }
    ...
    private PaymentForCallback lockPaymentByMerchantTxnRef(String merchantTxnRef) {
      ...
      private void recordPaymentEvent(UUID paymentId, String eventType, Map<String, ?> payload) {
          try {
              bookingOperationsRepository.insertPaymentEvent(UUID.randomUUID(), paymentId, eventType, JSON.writeValueAsString(payload));
    ...
    private UUID existingCheckInId(UUID bookingId) {
        return bookingOperationsRepository.existingCheckInId(bookingId);
    }

    private int bookedCapacity(UUID bookingId) {
        Integer capacity = bookingOperationsRepository.queryForObject("""
    ...
    private void insertGuest(UUID checkInId, CheckInGuestRequest guest, boolean primary, int order) {
        bookingOperationsRepository.insertGuest(
                checkInId,
                trimRequired(guest.fullName(), "Guest fullName is required"),
                trimToNull(guest.identityNumber()),
                trimToNull(guest.phone()),
                primary,
                order
        );
    }
    ...
    private void requireAction(UUID accountId, String actionKey, UUID hotelId) {
        ...
        Boolean allowed = bookingOperationsRepository.queryForObject("""
                select exists (
                    select 1
                    from api_actions a
    ...
    private boolean isAdmin(UUID accountId) {
        Boolean admin = bookingOperationsRepository.queryForObject("""
                select exists (
                    select 1
                    from account_roles ar
                    join roles r on r.id = ar.role_id
